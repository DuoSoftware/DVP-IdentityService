...
let User = require('dvp-mongomodels/model/User');
let UserAccount = require('dvp-mongomodels/model/UserAccount');
...
let Tenant = require('dvp-mongomodels/model/Tenant').Tenant;
let messageFormatter = require('dvp-common/CommonMessageGenerator/ClientMessageJsonFormatter.js');
let Org = require('dvp-mongomodels/model/Organisation');
let orgService = require('./OrganisationService');
...
let ADService = require('./ActiveDirectoryService');
let redisClient = require("../helpers/RedisHelper");
let Login = require("../Login");

/////////////////security breach///////////////////////
let commonsignature = config.auth.common_signature;

let multilogin = config.auth.multi_login;
...
class MongoLogin extends Login{
    ...
    Login(req, res) {
        ...
        User.findOne({"username": req.body.userName}, '+password', function (err, user) {
            ...
            let companyReg = ["^",req.body.companyName,"$"].join('');
    ...
    SignUP(req, res) {
        ...
        if(config.auth.signup_verification == true || config.auth.signup_verification == 'true') {
            ...
            let secretKey = config.auth.recaptcha_key;
                                    ...
                                    crypto.randomBytes(20, function (err, buf) {
                                        ...
                                        redisClient.set("activate" + ":" + token, result._id, function (err, val) {
                                            if (err) {
                                                ...
                                                redisClient.expireat("activate" + ":" + token, parseInt((+new Date) / 1000) + 86400);
                                                //let token = GetJWT(result, ["all_all"]);
                                ...
                                if (!err && payload) {
                                    User.findOne({username: payload.iss}, function (err, user) {
                                        ...
                                        user.locale = user.locale || profile.locale,
                                            user.save(function () {
                                                //let token = GetJWT(user,claims_arr);
                            ...
                            if (!user) {
                                User.findOne({"googleplus.cid": profile.sub}, function (err, existingUser) {
                                    if (existingUser) {
                                        ...
                                        let defaultTimezone = {tz: "", utcOffset: ""};
                                        ...
                                        if (!err) {
                                            ...
                                            orgService.CreateOrganisationStanAlone(user,profile.email, defaultTimezone, function (err, rUser) {
                                                ...
                                                if (!err && rUser) {
                                                    ...
                                                    //let token = GetJWT(rUser,claims_arr);
                                                    //res.send({state: "new", token: token});
            ...
            User.findOne({"email.contact": req.body.email}, function (err, existingUser) {
                if (!existingUser || err) {
                    ...
                    crypto.randomBytes(20, function (err, buf) {
                        ...
                        let url = config.auth.ui_host + '#/reset/' + token;
